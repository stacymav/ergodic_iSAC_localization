...
namespace sac {
	...
	/*********************************************
	/* Initial condition */
	const double x_init[] = {0.8, 0.2, 0.85, 0, 0, 0, 0, 0, 0, 0, 0, 0};
	...
	const double t_final = 30.0; // final time

	const double T = 1.3; // prediction horizon
	...
	const bool save_actions_to_file = false;
	...
	inline void u_nominal( const double t, const state_type & x, state_type & u_nom_t ) {
		for ( size_t i=0; i<ulen; ++i ) { 
			u_nom_t[i] = Kp*(des_height - x[2]) - Kd * x[8];
			//Saturation
