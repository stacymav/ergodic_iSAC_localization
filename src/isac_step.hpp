...
namespace sac {
  ...
  public:
    ...
    isac_step( std::vector<target> &r_mytargets ) : update_phik_count(1),
		 erg(r_mytargets), x(xlen), rho(xlen), u_switch(ulen), 
		 ...
		 { 	
			initialize_meas_noise( );//initialize measurement noise
		 }
    ...
    inline void operator() ( double &t0, const state_type &x0) {
	  ...
	  if(update_phik_count > update_steps) {
		erg.update_phik( t0 );
		update_phik_count = 1;
	  }
